...
def	makeKernel():
	...
	kernel_dir=kd[1]
	os.chdir(kernel_dir)
	os.system("export PATH="+ TC_DIR + '/' + tcbd + '/bin' + ':' + '$PATH'+ ';' + "time make ARCH=arm64 CROSS_COMPILE=aarch64-buildroot-linux-gnu-  O=" +TOP+"/obj/kernel-build -j2")
...
def     config_kernel_minimal():
	...
	os.system("export PATH="+ TC_DIR + '/' + tcbd + '/bin' + ':' + '$PATH'+ ';'+"make ARCH=arm64 CROSS_COMPILE=aarch64-buildroot-linux-gnu-  O=" +TOP+"/obj/kernel-build olddefconfig")
...
def minimalUserland():
	...
	cmd='export PATH='+ TC_DIR + '/' + tcbd + '/bin' + ':' + '$PATH'+ ';'+'make O=' + barm_path + ' ARCH=arm64 CROSS_COMPILE=aarch64-buildroot-linux-gnu-  defconfig'
	print(cmd)
	...
	if rc!=0:
		...
		print(bb2.stdout)
		os.chdir(barm_path)
		cmd2='export PATH='+ TC_DIR + '/'+ tcbd + '/bin' + ':' + '$PATH'+ ';'+ 'make -j2 ARCH=arm64 CROSS_COMPILE=aarch64-buildroot-linux-gnu-'
		bb3=subprocess.run(cmd2,shell=True,check=True, stdout=subprocess.PIPE, universal_newlines=True)
#		print(bb3.stdout)
		cmd3='export PATH='+ TC_DIR + '/' + tcbd + '/bin' + ':' + '$PATH'+ ';'+ 'make ARCH=arm64 CROSS_COMPILE=aarch64-buildroot-linux-gnu-  install'
...
if __name__ =='__main__':
	...
	createWorkArea()
	print("Downloading cross toolchain from Bootlin \n")
	DownloadBootlinToolchain()
